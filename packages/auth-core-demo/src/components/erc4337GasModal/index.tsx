...
import { formatPng, isMobile } from '../../utils';
...
const Erc4337GasModal = () => {
    ...
    useEffect(() => {
        if (feeQuotes) {
            if (feeQuotesResponse?.verifyingPaymasterGasless) {
                ...
                for (const feeQuote of feeQuotes) {
    ...
    const feeTokenBalance = (feeQuote: FeeQuote) => {
        return ethers.formatUnits(feeQuote.balance, feeQuote.tokenInfo.decimals);
    };

    const formatFeeQuote = (feeQuote: FeeQuote) => {
        try {
            return `-${ethers.formatUnits(feeQuote.fee, feeQuote.tokenInfo.decimals)}`;
        } catch (error) {
            console.error(error);
            return '';
        }
                        ...
                        return (
                            ...
                            >
                                ...
                                <Image
                                    className="fee-token-icon"
                                    preview={false}
                                    src={formatPng(feeQuote.tokenInfo.logoURI)}
                                ></Image>
